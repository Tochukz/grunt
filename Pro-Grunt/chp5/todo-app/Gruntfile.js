...
module.exports = function(grunt) {
    ...
    grunt.initConfig({
        ...
        // grunt-contrib-imagemin for image compression support
        imagemin: {
            app: {
                options: {
                    optimizationLevel: 6, // controls how many trials are run to compress the IDAT chuncks of PNG images.
                    progressive: true,
                    use: [pngquant()], //NodeJS module to be used for the compression
                },
                files: [{
                    expand: true,
                    cwd: 'app/',
        ...
        // grunt-responve-images fro resizes an image to various sizes
        responsive_images: {
            dest: {
                options: {
                    sizes: [
                      {
                        wudth: 320,
                        height: 240 // dimension will be used as the name suffix e.g background_
                      }, 
                      {
                        name: 'large', // name will be used as the name suffix e.g background-large.jpg
                        width: 640
                      }, 
                      {
                        name: "large",
                        width: 1024,
                        suffix: "_x2", // an additional suffix after the name value i.e background-large_x2.jpg
                        quality: 60 , //values between 1 and 100, where 100 is best-quality image.
                      }, 
                    ] /*other size object keys includes: rename, aspectRatio(default is true), 
                            gravity(position of image during cropping, possible values - northwest, north, northest etc. default is center), 
                            upscale(should the image be upscaled if the dimension is large than the original?), 
                            separator(charater used bewteeen the file name and the sufix) and 
                            units(provides mapping among pixels, percentages and multiply units and therir representation.  default options are {percentage: 'pc}, pixel: '', multiply: 'x)
                     */
                },
                files: [{
                    expand: true,
...
The grunt-contrib-imagemin is used to support compression of images. It wraps the NodeJS modeul imagemin, therefore, any imagemin module can be used with iagemin
The configuration options are explaind as follows:
 options.optimazationlevel => It controls how many trials are run to compress the IDAT chuncks of PNG images.
                              It accpets values from 0(no compression) to 7 (240 trials).
                              This is primarily a feature of the PNG compression tool included with the plug-in, known as optipng.
                              IDAT chunks are essentially the image data for a PNG file.
 options.progressive => This option allows us to affect how jpegtran, the JPEG compression tool used by grunt-contib-imagemin is used.
                        When this option is enabled, true, the resulting JPEG file will be progressive JPEG format.
                        Progressive JPEGs render, as the name suggests, progressively.
                        This means a low-quality version is initially visible, then, as more data is received over the network, the image quality will gragually improve.

The grunt-responsive-images plugin wraps the image processing tools GraphicsMagick and ImageMagick and allows a set of sizes by which to resize the original image.
*/
